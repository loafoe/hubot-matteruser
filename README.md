...
To upgrade your Hubot for Mattermost 4.4.x, find the `package.json` file in your Hubot directory and look for the line in the `dependencies` section that references `hubot-matteruser`. Change the verion so it points to `^5.4.2` of the client. Example:

  ```json
    ...
    "dependencies": {
      "hubot-matteruser": "^5.4.2"
    },
    ...
  ```
